...
Et voilà, vous avez exposé Vaultwarden ! Accédez au panneau d'administration via `https://vault.mondomaine.fr/admin` et collez le `ADMIN_TOKEN` que vous avez spécifié dans le `compose.yaml`. Plus d'info sur les fonctionnalités de [Bitwarden](https://bitwarden.com/help/).
